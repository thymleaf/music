...
public class RequestInterceptor implements Interceptor
{
    ...
    private String printResult(Request request, Response originalResponse) throws IOException
    {
        ...
        if (isParseable(responseBody.contentType()))
        {
            ...
            Timber.tag(getTag(request, "Response_Result"))
                  .w(isJson(responseBody.contentType()) ? CharactorUtil
                          .jsonFormat(bodyString) : isXml(
                          responseBody.contentType()) ? CharactorUtil
                          .xmlFormat(bodyString) : bodyString);
    ...
    private String parseContent(ResponseBody responseBody, String encoding, Buffer clone)
    {
        ...
        {//content使用zlib压缩
            return ZipHelper
                    .decompressToStringForZlib(clone.readByteArray(), convertCharset(charset));//解压
        }
        else
        {//content没有被压缩
    ...
    public static String parseParams(RequestBody body) throws UnsupportedEncodingException
    {
        if (isParseable(body.contentType()))
        {
            try
            {
                ...
                return URLDecoder
                        .decode(requestbuffer.readString(charset), convertCharset(charset));
    ...
    public static boolean isParseable(MediaType mediaType)
    {
        ...
        return mediaType.toString().toLowerCase().contains("text") || isJson(mediaType) ||
